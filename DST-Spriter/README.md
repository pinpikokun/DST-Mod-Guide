...
│   ├── hud/                        ←   HUD（クラフトタブ等）
│   ├── map_icons/                  ←   マップ上のアイコン
│   ├── saveslot_portraits/         ←   セーブスロットの肖像画
│   └── selectscreen_portraits/     ←   キャラクター選択画面
│
├── bigportraits/                   ← キャラクター選択時の大きい肖像画
│
├── scripts/
...
| **投射物** | 飛んでいくもの（矢・弾等） | `my_projectile.zip` | [README-effect.md](README-effect.md) |
| **範囲表示** | 射程リングなどの UI 表示 | `my_range_ring.zip` | [README-effect.md](README-effect.md) |
...
| セーブスロット肖像画 | `images/saveslot_portraits/` | 256 x 256 | セーブ画面 |
| キャラ選択画面 | `images/selectscreen_portraits/` | 512 x 512 | キャラ選択 |
| 大きい肖像画 | `bigportraits/` | 1024 x 1024 | キャラ選択（詳細） |
| MOD アイコン | ルート直下 | 128 x 128 | Steam ワークショップ |

> **Note**: すべて正方形・2 のべき乗サイズにしてください。サイズが合わないとクラッシュします。
