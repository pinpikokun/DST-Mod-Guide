...
4. **以下の「完成報告テンプレート」の 4 セクションをすべて人間に表示する。省略してはならない。**

### 完成報告テンプレート（必須出力）

Phase 4 が完了したら、以下の 4 セクションを **すべて** 人間に向けて出力すること。

#### セクション 1: MOD フォルダ構成

MOD フォルダのフルパスと、全ファイルのツリー構成を表示する。各ファイルの役割を簡潔にコメントで付記する。

```
<MOD フォルダのフルパス>\
├── modinfo.lua                              ← MOD 基本情報
├── modmain.lua                              ← メインスクリプト
├── modicon.tex / .xml                       ← MOD アイコン
├── anim/
│   ├── <キャラクター名>.zip                  ← キャラクターアニメーション
│   └── ghost_<キャラクター名>_build.zip      ← ゴーストアニメーション
├── bigportraits/<キャラクター名>.tex / .xml   ← 大ポートレート
├── images/
│   ├── avatars/avatar_<キャラクター名>.tex / .xml      ← アバター
│   ├── avatars/avatar_ghost_<キャラクター名>.tex / .xml ← ゴーストアバター
│   ├── map_icons/<キャラクター名>.tex / .xml            ← マップアイコン
│   ├── saveslot_portraits/<キャラクター名>.tex / .xml   ← セーブスロット
│   └── selectscreen_portraits/<キャラクター名>.tex / .xml ← 選択画面
├── scripts/prefabs/<キャラクター名>.lua       ← キャラクター定義
└── scripts/speech_<キャラクター名>.lua        ← セリフ
```

#### セクション 2: キャラクター設定

依頼された設定値が正しく反映されていることを表形式で示す。
...
1. DST を起動 →「ゲームをホスト」→「新しい世界を作成する」
2. 「MOD」タブで「（MOD 名）」を有効にする
3. キャラクター選択画面で「（表示名）」を選んでゲーム開始

#### セクション 4: 画像の差し替え案内

ダミー画像を使用しているため、オリジナル画像への差し替え方法を案内する。

- **ポートレート画像**: `work\<キャラクター名>\portraits\` 内の PNG を画像編集ソフトで編集し、「画像を差し替えたから TEX に変換して」と指示してください。
- **アニメーション画像**（顔・髪など）: `work\<キャラクター名>\` 内の `face/`、`hair/` 等のフォルダの PNG を編集し、「画像を差し替えたからアニメーション ZIP を再ビルドして」と指示してください。
