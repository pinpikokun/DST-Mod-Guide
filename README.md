...
- PNG から TEX ファイルへの変換（[DST-TextureConverter ガイド](DST-TextureConverter/README.md) を参照）
- アニメーションの詳細（[DST-Spriter ガイド](DST-Spriter/README.md) を参照）
  - 武器・アイテムの追加（[武器・アイテムガイド](DST-Spriter/README-item-equipment.md) を参照）
  - エフェクトの作成（[エフェクトガイド](DST-Spriter/README-effect.md) を参照）
- サウンドの追加（[DST-FMOD-Designer ガイド](DST-FMOD-Designer/README.md) を参照）
  - AI 音声合成によるボイス生成（[DST-Chatterbox ガイド](DST-Chatterbox/README.md) を参照）
- Lua スクリプティング — キャラクター固有能力・カスタムコンポーネント・レシピ追加など（作成中）
...
1. `sample_build` フォルダを **丸ごとコピー** して、`my_character` フォルダの隣に配置します。

   コピー先:
   ```
   C:\Program Files (x86)\Steam\steamapps\common\Don't Starve Mod Tools\mod_tools\my_character_build\
   ```
...
2. コピーしたフォルダ内の **`build.scml`** を Spriter で開いて、パーツ画像の配置を確認します。
   ...
   3. ファイル選択ダイアログが開くので、以下のファイルを選択して **「開く」** をクリックします
      ```
      C:\Program Files (x86)\Steam\steamapps\common\Don't Starve Mod Tools\mod_tools\my_character_build\build.scml
      ```
...
1. Step 6 で作成した `my_character_build` フォルダを以下の場所に移動します。

   ```
   C:\Program Files (x86)\Steam\steamapps\common\Don't Starve Mod Tools\mod_tools\exported\my_character_build\
   ```

   > **重要**: `exported` フォルダの中に置いてください。`exported` フォルダが無い場合は作成してください。
...
2. `autocompiler.exe` を実行します。
   ...
   > コマンドプロンプトの黒い画面が表示されます。処理が完了するまで待ってください。
...
> **画像サイズの重要ルール**: すべての画像サイズは **2 の累乗**（64, 128, 256, 512...）でなければなりません。それ以外のサイズ（例: 100 × 100）を使うと **ゲームがクラッシュ** します。

#### PNG の準備

1. 上の表に合わせて PNG 画像を作成します。
2. すべて **透過 PNG（RGBA 32bit）** で保存します。
3. 画像サイズは **必ず 2 の累乗** にしてください。
...
> - `-i`: 入力ファイル（PNG）
> - `-o`: 出力ファイル（TEX）
> - `-f bc3`: 圧縮形式（BC3 = DXT5、透過あり）
> - `-p opengl`: プラットフォーム
> - `--mipmap`: ミップマップ生成（必須）
> - `--premultiply`: アルファ値の事前乗算（必須）
...
4. [DST-FMOD-Designer ガイド](DST-FMOD-Designer/README.md) で **キャラクターボイス** を追加する
5. [DST-Chatterbox ガイド](DST-Chatterbox/README.md) で **AI 音声** を作成する
...
MOD が完成したら、Steam Workshop に公開して他のプレイヤーに遊んでもらえます。

1. Steam で **Don't Starve Mod Tools** を起動
2. **Mod Uploader** を選択
3. MOD フォルダを指定して、タイトル・説明・タグ・プレビュー画像を設定
4. **Upload** をクリック

> **公開前チェック**: `modinfo.lua` の `name` と `description` が英語で書かれていると、海外のプレイヤーにも見つけてもらいやすくなります。
